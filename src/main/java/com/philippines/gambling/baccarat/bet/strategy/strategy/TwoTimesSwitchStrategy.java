 ...
 */
public class TwoTimesSwitchStrategy extends StrategyConstants implements BetStrategy {
	
	@Override
	public BetResultEnum getBetWho(BetResultEnum ...onRound) {
		return twoTimesSwitchStrategy(onRound);
	}
	@Override
	 ...
	 */
	private static BetResultEnum[] previousTwoResult = new BetResultEnum[2];
	protected static BetResultEnum twoTimesSwitchStrategy(BetResultEnum ...onRound) {
		BetResultEnum currentRoundBet = onRound[0];
		if(currentRoundBet == null) {
			return null;
		}
		previousTwoResult[0] = previousTwoResult[1];
		previousTwoResult[1] = currentRoundBet;
